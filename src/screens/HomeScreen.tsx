...
import { demoNotifications, exercisesForProgramme, goalLabels, programmeForGoal } from '../product';
...
export function HomeScreen() {
  ...
  const { points, workoutSets, reservations, preferences, readNotificationIds, joinedChallenges } = useAppState();
  ...
  const loggedExercises = new Set(workoutSets.filter(set => programme.exerciseIds.includes(set.exerciseId)).map(set => set.exerciseId)).size;
  const workoutPercent = Math.round((loggedExercises / Math.max(1, todayExercises.length)) * 100);
  ...
  return <Shell>
    <View style={s.topbar}>
      ...
      <View style={styles.topActions}>
        <Pressable accessibilityRole="button" accessibilityLabel="Abrir notificações" onPress={() => navigation.navigate('Notifications')} style={styles.topIcon}><Ionicons name="notifications-outline" size={19} color={colors.accentDark} />{unread > 0 ? <View style={styles.badge}><Text style={styles.badgeText}>{unread}</Text></View> : null}</Pressable>
    ...
    <View style={styles.greetingRow}>
      <View style={{ flex: 1 }}><Text style={styles.eyebrow}>HOJE · {goalLabels[preferences.goal].toUpperCase()}</Text><Text style={styles.greeting}>Pronto para continuar?</Text></View>
    ...
    <View style={styles.trainingHero}>
      ...
      <View style={styles.heroTopRow}><Text style={styles.heroEyebrow}>SEMANA 4 · {programme.title.toUpperCase()}</Text><Text style={styles.heroPercent}>{workoutPercent}%</Text></View>
      <Text style={styles.heroTitle}>{programme.dayTitle}</Text>
      <Text style={styles.heroSubtitle}>{programme.focus} · {loggedExercises === 0 ? 'sessão preparada para hoje.' : `${loggedExercises} de ${todayExercises.length} exercícios iniciados.`}</Text>
    ...
    <View style={styles.quickGrid}>
      ...
      <QuickTile icon="trophy-outline" label="Desafios" value={joinedChallenges.length ? `${joinedChallenges.length} ativos` : 'Explorar'} onPress={() => navigation.navigate('Challenges')} />
    </View>
    ...
    <Pressable accessibilityRole="button" onPress={() => navigation.navigate('Aulas')}>
      <Card style={styles.classCard}>
        ...
        <View style={styles.classContent}>
          <View style={s.between}><View style={{ flex: 1 }}><Text style={styles.className}>{nextClass.name}</Text><Text style={s.muted}>{nextClass.instructor} · {nextClass.category}</Text></View><Ionicons name="chevron-forward" size={19} color={colors.slate} /></View>
    ...
    <SectionHeader title="O teu ecossistema" />
    <View style={styles.ecosystemRow}><FeatureLink icon="fitness-outline" title="Apps & dispositivos" text="Health, Garmin, Strava e Wallet" onPress={() => navigation.navigate('Integrations')} /><FeatureLink icon="stats-chart-outline" title="Histórico" text="Sessões, volume e progressão" onPress={() => navigation.navigate('WorkoutHistory')} /></View>
  </Shell>;
}
...
function FeatureLink({ icon, title, text, onPress }: { icon: keyof typeof Ionicons.glyphMap; title: string; text: string; onPress: () => void }) {
  return <Pressable accessibilityRole="button" onPress={onPress} style={styles.featureLink}><View style={styles.featureIcon}><Ionicons name={icon} size={19} color={colors.accentDark} /></View><Text style={styles.featureTitle}>{title}</Text><Text style={styles.featureText}>{text}</Text><Ionicons name="arrow-forward" size={16} color={colors.accentDark} /></Pressable>;
}

const styles = StyleSheet.create({
  topActions: { flexDirection: 'row', alignItems: 'center', gap: 8 },
  topIcon: { width: 42, height: 42, borderRadius: 14, backgroundColor: colors.panel, borderWidth: 1, borderColor: colors.border, alignItems: 'center', justifyContent: 'center' },
  badge: { position: 'absolute', right: -2, top: -3, minWidth: 17, height: 17, paddingHorizontal: 4, borderRadius: 9, backgroundColor: colors.danger, alignItems: 'center', justifyContent: 'center' },
  ...
  heroEyebrow: { color: '#AED3ED', fontSize: 8, fontWeight: '900', letterSpacing: 1 },
  ...
  quickValue: { color: colors.text, fontSize: 13, fontWeight: '900' },
  ...
  ecosystemRow: { flexDirection: 'row', gap: 9 },
  featureLink: { flex: 1, minHeight: 132, backgroundColor: colors.panel, borderWidth: 1, borderColor: colors.border, borderRadius: 18, padding: 13, alignItems: 'flex-start' },
  featureIcon: { width: 36, height: 36, borderRadius: 12, backgroundColor: colors.accentSoft, alignItems: 'center', justifyContent: 'center' },
  featureTitle: { color: colors.text, fontSize: 11, fontWeight: '900', marginTop: 11 },
  featureText: { flex: 1, color: colors.muted, fontSize: 8, lineHeight: 13, marginTop: 3 },
});
