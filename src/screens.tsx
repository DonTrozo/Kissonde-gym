...
export function LoginScreen() {
  ...
  return <SafeAreaView style={styles.loginScreen}>
    <View style={styles.loginBrand}><BrandLogo width={220} /></View>
    <View style={styles.loginBody}>
      ...
      <TextInput value={memberId} onChangeText={setMemberId} autoCapitalize="characters" placeholder="KSG-00000" placeholderTextColor={colors.slate} style={styles.input} />
      <Text style={styles.inputLabel}>PALAVRA-PASSE</Text>
      <TextInput value={password} onChangeText={setPassword} secureTextEntry placeholder="••••••••" placeholderTextColor={colors.slate} style={styles.input} />
  ...
  return <Shell>
    ...
    <Card style={styles.heroCard}>
      <View style={styles.spaceBetween}>
        <View style={{ flex: 1 }}><Text style={styles.kicker}>ACESSO RÁPIDO</Text><Text style={styles.cardTitle}>Cartão digital</Text><Text style={styles.cardMuted}>Adesão {member.status.toLowerCase()} · disponível offline</Text></View>
    ...
    <Card>
      ...
      <View style={styles.progressTrack}><View style={[styles.progressFill, { width: `${workoutPercent}%` as any }]} /></View>
    </Card>
...
export function AccessScreen() {
  return <Shell>
    ...
    <Card style={{ alignItems: 'center', gap: 16 }}>
      <View style={styles.qrWrap}><QRCode value={`KISSONDE:${member.id}:ACTIVE`} size={190} backgroundColor="#FFFFFF" color={colors.text} /></View>
    ...
    {visits.map(v => <Card key={v.id} style={{ marginBottom: 10 }}>
      <View style={styles.spaceBetween}><View style={{ flex: 1 }}><Text style={styles.cardTitle}>{v.date} · {v.branch}</Text><Text style={styles.cardMuted}>{v.enteredAt}{v.exitedAt ? ` — ${v.exitedAt}` : ''} · ID {v.id}</Text></View><Pill tone={v.status === 'verified' ? 'success' : 'info'}>{v.status === 'verified' ? `Verificada +${v.points}` : 'Pendente'}</Pill></View>
    </Card>)}
...
export function TrainScreen() {
  ...
  return <Shell>
    ...
    {exercises.map((exercise, index) => {
      ...
      return <Card key={exercise.id} style={[{ marginBottom: 10 }, done ? styles.doneCard : undefined]}>
        <View style={styles.exerciseRow}>
          ...
          <Pressable onPress={() => toggleExercise(exercise.id)} style={[styles.check, done && styles.checkDone]}><Ionicons name={done ? 'checkmark' : 'add'} size={20} color={done ? '#FFFFFF' : colors.text} /></Pressable>
        </View>
        <View style={styles.exerciseCompare}><View><Text style={styles.smallLabel}>ÚLTIMA SESSÃO</Text><Text style={styles.compareValue}>{exercise.previous}</Text></View><Ionicons name="arrow-forward" color={colors.slate} size={18} /><View><Text style={styles.smallLabel}>SUGERIDO HOJE</Text><Text style={styles.compareValue}>{exercise.suggested}</Text></View></View>
      </Card>;
    })}
    ...
    <Card>
      <View style={styles.metricsRow}><Metric value="82.5 kg" label="Melhor supino" /><Metric value="3.8x" label="Treinos/semana" /><Metric value="+12.5 kg" label="Força em 12 semanas" /></View>
      <Text style={styles.chartTitle}>Volume de treino · últimas 6 semanas</Text>
      <View style={styles.chart}>{[42, 55, 61, 58, 72, 84].map((h, i) => <View key={i} style={styles.barSlot}><View style={[styles.bar, { height: h }]} /><Text style={styles.barLabel}>S{i + 1}</Text></View>)}</View>
    </Card>

    <SectionHeader title="Biblioteca de exercícios" />
    {exercises.slice(0, 4).map(exercise => <Pressable key={`library-${exercise.id}`} onPress={() => Alert.alert(exercise.name, `Músculo principal: ${exercise.target}\n\nTécnica: mantém controlo total do movimento, amplitude confortável e progressão gradual de carga.\n\nA demonstração em vídeo será ligada ao catálogo de conteúdo Kissonde.`)} style={styles.libraryRow}>
      <View style={styles.libraryIcon}><Ionicons name="play" size={17} color={colors.accent} /></View>
      <View style={{ flex: 1 }}><Text style={styles.ledgerTitle}>{exercise.name}</Text><Text style={styles.cardMuted}>{exercise.target} · ver técnica e alternativas</Text></View>
  ...
  return <Shell>
    ...
    {classes.map(c => {
      ...
      return <Card key={c.id} style={{ marginBottom: 10 }}>
        <View style={styles.spaceBetween}><View style={{ flex: 1 }}><Text style={styles.cardTitle}>{c.time} · {c.name}</Text><Text style={styles.cardMuted}>{c.instructor} · {c.duration} · {c.difficulty}</Text></View><Pill tone={full ? 'danger' : 'success'}>{full ? `${c.capacity}/${c.capacity}` : `${c.capacity - c.booked} vagas`}</Pill></View>
...
export function RewardsScreen() {
  ...
  return <Shell>
    ...
    {ledger.map(item => <View key={item.id} style={styles.ledgerRow}><View><Text style={styles.ledgerTitle}>{item.title}</Text><Text style={styles.cardMuted}>{item.date}</Text></View><Text style={[styles.ledgerPoints, { color: item.points >= 0 ? colors.accent : colors.danger }]}>{item.points >= 0 ? '+' : ''}{item.points}</Text></View>)}
  </Shell>;
}
...
export function ProfileScreen() {
  ...
  return <Shell>
    <View style={styles.profileBrand}><BrandLogo width={132} /></View>
...
export function SupportScreen() {
  ...
  return <Shell>
    <ScreenTitle eyebrow="Suporte" title="Resolver sem adivinhar." subtitle="O pedido inclui automaticamente o teu ID de membro e referências relevantes." />
    {options.map(option => <Pressable key={option} onPress={() => Alert.alert('Pedido preparado', `${option}\nMembro: ${member.id}\nClube: ${member.branch}`)} style={styles.supportRow}><Text style={styles.supportText}>{option}</Text><Ionicons name="chevron-forward" size={20} color={colors.slate} /></Pressable>)}
  </Shell>;
}
...
function Setting({ label, value, onChange, last = false }: { label: string; value: boolean; onChange: (value: boolean) => void; last?: boolean }) {
  return <View style={[styles.infoRow, last && { borderBottomWidth: 0 }]}><Text style={styles.infoValue}>{label}</Text><Switch value={value} onValueChange={onChange} trackColor={{ false: colors.border, true: '#8EBDE0' }} thumbColor={value ? colors.accent : colors.slate} /></View>;
}

const styles = StyleSheet.create({
  loginScreen: { flex: 1, backgroundColor: colors.bg, padding: 22, justifyContent: 'space-between' },
  loginBrand: { flex: 1, alignItems: 'center', justifyContent: 'center', minHeight: 220 },
  loginBody: { paddingBottom: 30 },
  inputLabel: { color: colors.muted, fontSize: 10, fontWeight: '900', letterSpacing: .9, marginBottom: 7, marginTop: 8 },
  ...
  demoNote: { color: colors.muted, fontSize: 11, lineHeight: 17, textAlign: 'center', marginTop: 14 },

  topbar: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginBottom: 22 },
  profileBrand: { alignItems: 'flex-start', marginBottom: 18 },
  ...
  heroCard: { gap: 16, borderTopWidth: 3, borderTopColor: colors.accent },
  ...
  time: { color: colors.accentDark, fontSize: 16, fontWeight: '900' },
  occupancyTrack: { height: 8, backgroundColor: colors.panel2, borderRadius: 99, overflow: 'hidden', marginTop: 16 },
  occupancyFill: { height: '100%', backgroundColor: colors.accentDark },

  qrWrap: { padding: 18, backgroundColor: '#FFFFFF', borderRadius: 22, borderWidth: 1, borderColor: colors.border },
  memberName: { color: colors.text, fontSize: 22, fontWeight: '900' },
  programCard: { gap: 8, borderTopWidth: 3, borderTopColor: colors.accentDark },
  doneCard: { borderColor: colors.accent, backgroundColor: '#F5FAFE' },
  ...
  check: { width: 36, height: 36, borderRadius: 18, borderWidth: 1, borderColor: colors.border, backgroundColor: colors.panel, alignItems: 'center', justifyContent: 'center' },
  ...
  chartTitle: { color: colors.muted, fontSize: 11, fontWeight: '800', marginTop: 22, marginBottom: 12 },
  chart: { height: 115, flexDirection: 'row', alignItems: 'flex-end', gap: 10 },
  barSlot: { flex: 1, alignItems: 'center', justifyContent: 'flex-end', gap: 5 },
  bar: { width: '72%', backgroundColor: colors.accent, borderRadius: 6 },
  barLabel: { color: colors.muted, fontSize: 9 },
  libraryRow: { flexDirection: 'row', alignItems: 'center', gap: 12, minHeight: 68, paddingHorizontal: 14, marginBottom: 8, borderRadius: radius.md, backgroundColor: colors.panel, borderWidth: 1, borderColor: colors.border },
  libraryIcon: { width: 38, height: 38, borderRadius: 19, backgroundColor: '#E8F2FA', alignItems: 'center', justifyContent: 'center' },
  ...
  trainerAvatar: { width: 50, height: 50, borderRadius: 25, backgroundColor: '#E8F2FA', alignItems: 'center', justifyContent: 'center' },
  ...
  pointsHero: { alignItems: 'center', paddingVertical: 26, borderTopWidth: 3, borderTopColor: colors.accent },
  points: { color: colors.accent, fontSize: 48, fontWeight: '900', letterSpacing: -2 },
  rewardCost: { color: colors.accentDark, fontSize: 20, fontWeight: '900' },
