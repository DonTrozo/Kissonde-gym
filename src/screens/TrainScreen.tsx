...
function ExerciseLogger({ exercise, order }: { exercise: Exercise; order: number }) {
  ...
  return <Card style={[styles.exerciseCard, complete && styles.exerciseCardComplete]}>
    <View style={styles.exerciseHeader}>
      ...
      <View style={styles.exerciseHeading}><Text style={styles.exerciseName}>{exercise.name}</Text><Text style={styles.exerciseMeta}>{exercise.target} · objetivo {exercise.sets} × {exercise.reps}</Text></View>
    ...
    <View style={styles.performanceStrip}>
      ...
      <View style={styles.performanceColumn}><Text style={styles.performanceLabel}>ALVO DE HOJE</Text><Text style={styles.performanceValue}>{exercise.suggested}</Text></View>
    </View>
...
function ProgressView() {
  ...
  return <>
    ...
    {exercises.map(exercise => <Card key={`library-${exercise.id}`} style={styles.libraryCard}><View style={styles.libraryIcon}><Ionicons name="play" size={15} color={colors.accentDark} /></View><View style={{ flex: 1 }}><Text style={s.cardTitle}>{exercise.name}</Text><Text style={s.muted}>{exercise.target} · técnica e alternativas</Text></View><Ionicons name="chevron-forward" size={17} color={colors.slate} /></Card>)}
  </>;
}
...
const styles = StyleSheet.create({
  ...
  sessionEyebrow: { color: '#A9D1ED', fontSize: 8, lineHeight: 12, fontWeight: '900', letterSpacing: 1 },
  ...
  tabs: { flexDirection: 'row', backgroundColor: colors.panel3, borderRadius: 15, padding: 4, marginTop: 12, marginBottom: 0 },
