...
export function Pill({ children, tone = 'default' }: { children: React.ReactNode; tone?: 'default' | 'success' | 'danger' | 'info' }) {
  const bg = tone === 'success' ? '#E8F2FA' : tone === 'danger' ? '#FDECEC' : tone === 'info' ? '#E7EFF7' : '#EEF2F6';
  const fg = tone === 'success' ? colors.accent : tone === 'danger' ? colors.danger : tone === 'info' ? colors.accentDark : colors.text;
...
export function PrimaryButton({ label, onPress, disabled = false }: { label: string; onPress: () => void; disabled?: boolean }) {
  return <Pressable onPress={onPress} disabled={disabled} style={({ pressed }) => [styles.primary, disabled && { opacity: .45 }, pressed && !disabled && styles.primaryPressed]}><Text style={styles.primaryText}>{label}</Text></Pressable>;
}

export function SecondaryButton({ label, onPress }: { label: string; onPress: () => void }) {
  return <Pressable onPress={onPress} style={({ pressed }) => [styles.secondary, pressed && { backgroundColor: colors.panel2 }]}><Text style={styles.secondaryText}>{label}</Text></Pressable>;
}
...
const styles = StyleSheet.create({
  ...
  secondary: { minHeight: 44, paddingHorizontal: 16, borderRadius: radius.md, borderWidth: 1, borderColor: colors.border, backgroundColor: colors.panel, alignItems: 'center', justifyContent: 'center' },
