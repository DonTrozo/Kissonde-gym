...
export function TimerScreen() {
  ...
  const confirmReset = () => {
    if (timer.active.phase === 'complete' && !timer.active.running) return timer.reset();
    Alert.alert('Repor timer?', 'O intervalo ou cronómetro atual será terminado.', [{ text: 'Cancelar', style: 'cancel' }, { text: 'Repor', style: 'destructive', onPress: timer.reset }]);
  };
  ...
  return <Shell>
    ...
    <View style={styles.activeHero}>
      ...
      <Text style={styles.clock}>{formatTimerSeconds(displaySeconds)}</Text>
      ...
      <View style={styles.heroActions}>
        ...
        {timer.active.mode !== 'stopwatch' && timer.active.phase !== 'complete' ? <><HeroAction icon="add" label="+10s" onPress={() => timer.addSeconds(10)} /><HeroAction icon="add-circle-outline" label="+30s" onPress={() => timer.addSeconds(30)} /></> : null}
        {timer.active.mode === 'stopwatch' && timer.active.phase !== 'complete' ? <HeroAction icon="flag-outline" label="Volta" onPress={timer.addLap} /> : null}
        {timer.active.mode !== 'stopwatch' && timer.active.phase !== 'complete' ? <HeroAction icon="play-skip-forward" label="Saltar" onPress={timer.skipPhase} /> : null}
        <HeroAction icon="refresh" label="Reset" onPress={confirmReset} />
      </View>
    </View>
    ...
    {tab === 'rest' ? <>
      ...
      <Card>
        ...
        <View style={styles.buttonGap}><SecondaryButton label={`Usar ${Number(restSeconds) || 90}s como padrão`} onPress={() => timer.updateSettings({ defaultRestSeconds: Number(restSeconds) || 90 })} /></View>
      </Card>
    </> : null}
    ...
    {tab === 'simple' ? <>
      ...
      <Card style={styles.stopwatchCard}><View style={styles.stopwatchIcon}><Ionicons name="stopwatch-outline" size={24} color={colors.accentDark} /></View><View style={{ flex: 1 }}><Text style={s.cardTitle}>Cronómetro livre</Text><Text style={s.muted}>Para pranchas, mobilidade, cardio, holds e testes.</Text></View><Pressable accessibilityRole="button" accessibilityLabel="Iniciar cronómetro" onPress={timer.startStopwatch} style={styles.playCircle}><Ionicons name="play" size={18} color={colors.white} /></Pressable></Card>
      {timer.active.mode === 'stopwatch' && timer.active.laps.length > 0 ? <><SectionHeader title="Voltas" action={`${timer.active.laps.length}`} /><Card style={styles.lapCard}>{[...timer.active.laps].reverse().map((lap, index) => <View key={`${lap}-${index}`} style={[styles.lapRow, index === timer.active.laps.length - 1 && { borderBottomWidth: 0 }]}><Text style={styles.lapLabel}>Volta {timer.active.laps.length - index}</Text><Text style={styles.lapTime}>{formatTimerSeconds(lap)}</Text></View>)}</Card></> : null}
    </> : null}
...
function HeroAction({ icon, label, onPress }: { icon: keyof typeof Ionicons.glyphMap; label: string; onPress: () => void }) { return <Pressable accessibilityRole="button" onPress={onPress} style={styles.heroAction}><Ionicons name={icon} size={17} color={colors.white} /><Text style={styles.heroActionText}>{label}</Text></Pressable>; }
...
const styles = StyleSheet.create({
  ...
  clock: { color: colors.white, fontSize: 58, lineHeight: 64, fontWeight: '900', letterSpacing: -2.4, textAlign: 'center', marginTop: 18 },
  ...
  heroAction: { minWidth: 70, minHeight: 42, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 6, backgroundColor: '#246F9F', borderRadius: 13, paddingHorizontal: 9 },
  ...
  lapCard: { paddingVertical: 3 },
  lapRow: { minHeight: 44, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', borderBottomWidth: 1, borderBottomColor: colors.border },
  lapLabel: { color: colors.muted, fontSize: 10, fontWeight: '800' },
  lapTime: { color: colors.text, fontSize: 13, fontWeight: '900' },
